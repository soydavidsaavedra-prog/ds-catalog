...
import { createBannerAction, deleteBannerAction, updateBannerAction } from "@/app/[tenant]/admin/actions";
...
export default async function AdminBannersPage({
  ...
  const banners = await listBanners(tenant.id);
  const createAction = createBannerAction.bind(null, tenant.id, tenantSlug);
      ...
      <section className="flex flex-col gap-4">
        {banners.map((banner) => (
          <div key={banner.id} className="rounded-card border border-border bg-surface-elevated p-5">
            <form action={updateBannerAction.bind(null, tenant.id, tenantSlug, banner.id)} className="grid gap-4 sm:grid-cols-2">
              <div>
              ...
              <div className="flex items-center gap-4 sm:col-span-2">
                ...
                <NSAdminDeleteButton
                  action={deleteBannerAction.bind(null, tenant.id, tenantSlug, banner.id)}

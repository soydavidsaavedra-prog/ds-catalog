import type { Metadata } from "next";
import Link from "next/link";
import { DSPlatformMark } from "@/components/brand/DSPlatformMark";
import { NSAccederForm } from "@/components/registro/NSAccederForm";

export const metadata: Metadata = {
  title: "Acceder",
  robots: { index: false, follow: false },
};

export default function AccederPage() {
  return (
    <div className="flex min-h-dvh items-center justify-center bg-ink-950 px-4 text-ink-0">
      <div className="w-full max-w-sm">
        <div className="mb-8 flex flex-col items-center gap-3 text-center">
          <DSPlatformMark className="h-14 w-14" />
          <div>
            <p className="font-display text-xl uppercase tracking-wide">Acceder a tu panel</p>
            <p className="text-xs text-ink-400">Entra con el enlace de tu catálogo y tu contraseña.</p>
          </div>
        </div>
        <div className="rounded-card border border-ink-800 bg-ink-900 p-6">
          <NSAccederForm />
        </div>
        <p className="mt-6 text-center text-xs text-ink-500">
          ¿No tienes catálogo?{" "}
          <Link href="/registro" className="font-semibold text-accent hover:underline">
            Crea el tuyo
          </Link>
        </p>
      </div>
    </div>
  );
}

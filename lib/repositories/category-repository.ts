...
export async function listCategories(tenantId: string, opts?: { activeOnly?: boolean }): Promise<Category[]> {
  const supabase = getSupabaseClient();
  let query = supabase.from("ns_categories").select("*").eq("tenant_id", tenantId).order("order", { ascending: true });
...
export async function getDescendantSlugs(tenantId: string, slug: string): Promise<string[]> {
...
export async function createCategory(tenantId: string, input: CategoryInput): Promise<Category> {
  const supabase = getSupabaseClient();
  const nextOrder = input.order ?? (await listCategories(tenantId)).length + 1;
...
export async function deleteCategory(tenantId: string, id: string): Promise<void> {
  const supabase = getSupabaseClient();
  const { error } = await supabase.from("ns_categories").delete().eq("tenant_id", tenantId).eq("id", id);
  if (error) throw error;
}
...
export async function reorderCategories(tenantId: string, orderedIds: string[]): Promise<void> {
  ...
  await Promise.all(
    orderedIds.map((id, index) =>
      supabase.from("ns_categories").update({ order: index + 1 }).eq("tenant_id", tenantId).eq("id", id),
    ),
  );
}
